...
void ImuProcess::IMU_Initial(const MeasureGroup &meas, StatesGroup &state_inout, int &N)
{
    ...
    // TODO: fix the cov
    // cov_acc = Eigen::Vector3d(COV_START_ACC_DIAG, COV_START_ACC_DIAG, COV_START_ACC_DIAG); // 这里cov_acc与cov_gyr不就白算了吗?
    // cov_gyr = Eigen::Vector3d(COV_START_GYRO_DIAG, COV_START_GYRO_DIAG, COV_START_GYRO_DIAG);
    state_inout.gravity = -mean_acc / mean_acc.norm() * G_m_s2;
    // state_inout.gravity = Eigen::Vector3d(0, 0, 9.805);
    state_inout.bias_g = mean_gyr;
    // state_inout.rot_end = Eye3d;
    state_inout.rot_ext_l2i = Lidar_R_wrt_IMU;
    state_inout.pos_ext_l2i = Lidar_T_wrt_IMU;
...
// fout<<head->header.stamp.toSec()<<" "<<angvel_avr.transpose()<<" "<<acc_avr.transpose()<<std::endl;
    ...
    for (auto it_kp = IMU_pose.end() - 1; it_kp != IMU_pose.begin(); it_kp--)
    {
        ...
        for (; it_pcl->curvature / double(1000) > head->offset_time; it_pcl--)
        {
            ...
            Eigen::Vector3d P_compensate = state_inout.rot_ext_l2i.transpose() * (state_inout.rot_end.transpose() * (R_i * (state_inout.rot_ext_l2i * P_i + state_inout.pos_ext_l2i) + T_ei) - state_inout.pos_ext_l2i);
