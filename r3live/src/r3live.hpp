...
const int laserCloudWidth = 48; // 设置cube的边长
const int laserCloudHeight = 48;
const int laserCloudDepth = 48;
const int laserCloudNum = laserCloudWidth * laserCloudHeight * laserCloudDepth; // 设置cube中一共有多少个LiDAR点
...
extern std::shared_ptr<ImuProcess> g_imu_process; // 无用
extern double g_lidar_star_tim; // 好像无用 // 当前LiDAR帧中第1个点的时间戳, 当前LiDAR帧时间戳
...
public:
    ...
    ros::Publisher pub_odom_cam;
    ros::Publisher pub_path_cam;
    ...
    int laserCloudCenWidth = 24; // 设置cube中边的中点位置
    int laserCloudCenHeight = 24;
    int laserCloudCenDepth = 24;

    int laserCloudValidNum = 0; // 好像无用
    ...
    bool flg_exit = false; // 无用
    bool flg_reset = false;

    // Buffers for measurements
    double cube_len = 0.0; // cube_side_length, cube侧边长度
    ...
    double copy_time;
    double readd_time;
    double fov_check_time;
    double readd_box_time;
    double delete_box_time;
    double kdtree_incremental_time;
    double kdtree_search_time;

    std::deque<sensor_msgs::PointCloud2::ConstPtr> lidar_buffer;
    std::deque<sensor_msgs::Imu::ConstPtr> imu_buffer_lio;
    std::deque<sensor_msgs::Imu::ConstPtr> imu_buffer_vio;

    PointCloudXYZINormal::Ptr featsFromMap; // surf feature in map
    PointCloudXYZINormal::Ptr cube_points_add;
    PointCloudXYZINormal::Ptr laserCloudFullRes2; // all points
    ...
    PointCloudXYZINormal::Ptr featsArray[laserCloudNum]; // 存储cube中的所有LiDAR点, 或理解为cube本身
    bool _last_inFOV[laserCloudNum];
    bool now_inFOV[laserCloudNum];
    bool cube_updated[laserCloudNum];
    int laserCloudValidInd[laserCloudNum]; // 无用
    ...
    bool flg_map_initialized = false; // 标志位, 判断地图ikdtree初始化是否成功
    int effect_feat_num = 0;
    int frame_num = 0;
    double filter_size_corner_min; // 无用
    ...
    double frame_first_pt_time = 0; // 出现LiDAR异常时, 异常LiDAR帧中第1个点的时间戳, 异常LiDAR帧时间戳
    double m_voxel_downsample_size_axis_z;
    geometry_msgs::PoseStamped msg_body_pose;
    nav_msgs::Odometry odomAftMapped;
    PointType pointOri; // 无用
    PointType pointSel;
    PointType coeff; // 无用
    pcl::VoxelGrid<PointType> downSizeFilterSurf;
    pcl::VoxelGrid<PointType> downSizeFilterMap;
    std::vector<double> m_initial_pose; // 无用
    ...
    std::deque<sensor_msgs::ImageConstPtr> m_queue_image_msg; // 无用
    ...
    int g_LiDAR_frame_index = 0; // 记录LiDAR帧数
    std::mutex g_mutex_render; // 无用
    std::shared_ptr<Image_frame> g_last_image_pose_for_render = nullptr; // 无用
    ...
    R3LIVE()
    {
        ...
        if (1)
        {
            ...
            get_ros_parameter(m_ros_node_handle, "r3live_common/map_output_dir", m_map_output_dir, Common_tools::get_home_folder().append("/r3live_output")); // 系统输出文件路径
        ...
        if (1)
        {
            ...
            get_ros_parameter(m_ros_node_handle, "r3live_lio/voxel_downsample_size_axis_z", m_voxel_downsample_size_axis_z, m_voxel_downsample_size_surf);
        ...
        m_thread_pool_ptr = std::make_shared<Common_tools::ThreadPool>(6, true, false); // 线程池, 至少需要5个线程, 这里分配6个线程
    ...
    // LIO子系统函数
    void imu_cbk(const sensor_msgs::Imu::ConstPtr &msg_in); // IMU回调函数
    void feat_points_cbk(const sensor_msgs::PointCloud2::ConstPtr &msg_in); // LiDAR特征回调函数
    bool get_pointcloud_data_from_ros_message(sensor_msgs::PointCloud2::ConstPtr &msg, pcl::PointCloud<pcl::PointXYZINormal> &pcl_pc); // 将ROS格式的点云转换为PCL格式的点云, 为什么不用PCL库函数, 而是要自定义

    void pointBodyToWorld(PointType const *const pi, PointType *const po); // 将LiDAR点从LiDAR坐标系变换到World坐标系
    void RGBpointBodyToWorld(PointType const *const pi, pcl::PointXYZI *const po); // 将LiDAR点从LiDAR坐标系变换到World坐标系, 同时根据强度信息生成颜色信息
    template <typename T>
    void pointBodyToWorld(const Eigen::Matrix<T, 3, 1> &pi, Eigen::Matrix<T, 3, 1> &po) // 好像无用
    {
    ...
    bool center_in_FOV(Eigen::Vector3f cube_p); // 无用
    bool if_corner_in_FOV(Eigen::Vector3f cube_p); // 无用
    void lasermap_fov_segment();
    void wait_render_thread_finish(); // LIO与VIO共用, 等待地图颜色渲染线程结束
    bool sync_packages(MeasureGroup &meas); // 同步数据, 将当前LiDAR帧与对应的若干帧IMU数据存储在结构体MeasureGroup中, 再将这些数据从各自队列中删除
    int service_LIO_update(); // LIO子系统线程
    ...
    void print_dash_board(); // LIO与VIO共用, 打印仪表盘
};
